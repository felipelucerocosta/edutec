...
CREATE UNIQUE INDEX IF NOT EXISTS curso_nombre_curso_key 
  ON public.curso (lower(nombre_curso));
...
CREATE UNIQUE INDEX IF NOT EXISTS materia_nombre_materia_key 
  ON public.materia (lower(nombre_materia));
...
CREATE TABLE IF NOT EXISTS public.usuarios (
  ...
  contrasena text NOT NULL, -- hash seguro (bcrypt/argon2)
...
CREATE UNIQUE INDEX IF NOT EXISTS usuarios_dni_key 
...
CREATE INDEX IF NOT EXISTS idx_profesor_id_usuario 
  ON public.profesor (id_usuario);
...
CREATE TABLE IF NOT EXISTS public.alumnos_clases (
  ...
  created_at timestamptz NOT NULL DEFAULT now(),
  updated_at timestamptz NOT NULL DEFAULT now(),
  CONSTRAINT uq_alumno_clase UNIQUE (id_alumno, id_clase)
);
...
CREATE TABLE IF NOT EXISTS public.material_clase (
  ...
  created_at timestamptz NOT NULL DEFAULT now(),
  updated_at timestamptz NOT NULL DEFAULT now(),
  CONSTRAINT material_clase_visibilidad_check 
...
CREATE UNIQUE INDEX IF NOT EXISTS alumno_correo_key 
  ON public.alumno (lower(coalesce(correo, '')));
